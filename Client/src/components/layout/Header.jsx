...
export function Header() {
  ...
  const { user, logout, sellerRequest, requestSeller } = useAuth();
                            ...
                            <p className={`text-xs truncate ${!conv.lastMessagePreview ? 'text-slate-400 italic' : 'text-slate-500'}`}>
                              {conv.lastMessagePreview ? (
                                <>
                                  ...
                                  {conv.lastMessagePreview.messageType === 2
          ...
          {user && (
            <div className="relative ml-2">
              <button
                ...
                className={`flex items-center cursor-pointer gap-2 p-1 pr-3 rounded-full transition-all border ${openProfile ? "bg-slate-900 border-slate-900 text-white" : "bg-white border-slate-200 text-slate-700 hover:border-slate-300"
                  }`}
              >
