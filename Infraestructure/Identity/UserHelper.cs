...
namespace Infraestructure.Identity
{
    public class UserHelper : IUserHelper
    {
        ...
        public string CreateJWTToken(AppUser user, IList<string> roles)
        {
            ...
            //issuer and audicence for creating totken
            //validateissuer and validate audience for validating token
            var token = new JwtSecurityToken(
                issuer: _configuration["Jwt:Issuer"],
                audience: _configuration["Jwt:Audience"],
                claims: claims,
                expires: DateTime.UtcNow.AddHours(double.Parse(_configuration["Jwt:ExpirationDurationInHour"]!)),
